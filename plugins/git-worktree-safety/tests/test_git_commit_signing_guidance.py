...
class GitCommitSigningGuidanceTests(unittest.TestCase):
    def test_frontmatter_anchors_trigger_and_negative_boundaries(self):
        ...
        for cue in (
            "ordinary git commit fails",
            "ssh signing",
            "signer, agent, socket",
            "staged state",
            "single-use signed retry",
            "commit, parent, diff, signer identity",
        ):
            self.assertIn(cue, frontmatter)
        for boundary in (
            "hook failures or hook-mutated state",
            "conflicts",
            "index or object corruption",
            "remote authentication",
            "openpgp or x.509 commit signing",
            "artifact or release signing",
            "key generation or rotation",
            "editor/template or cleanup-transformed commit messages",
            "initial or root commits",
            "amend, merge, empty commits",
            "index-altering `-a`/pathspec/interactive commit modes",
            "command-scoped git configuration or commit-metadata environment overrides",
            "rebase/cherry-pick sequencers",
