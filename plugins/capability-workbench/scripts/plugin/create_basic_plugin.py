...
def validate_existing_icon_path(plugin_root: Path, raw_path: str) -> str:
    candidate = PurePosixPath(raw_path.replace("\\", "/"))
    if candidate.is_absolute() or any(part in {"", ".", ".."} for part in candidate.parts):
        raise ValueError("--icon-path must be a relative path inside the plugin.")
    ...
    if not resolved.is_file():
        raise FileNotFoundError(f"Icon asset does not exist yet: {resolved}")
    return "./" + normalized.lstrip("./")


def validate_brand_color(brand_color: str) -> str:
    if re.fullmatch(r"#[0-9A-Fa-f]{6}", brand_color) is None:
        raise ValueError("--brand-color must use #RRGGBB.")
    return brand_color.upper()
...
def build_plugin_json(
    ...
    *,
    ...
    interface: dict[str, Any] = {
        ...
        "longDescription": f"{display_name} adds a Codex plugin scaffold.",
    ...
    if icon_path is not None:
        interface["composerIcon"] = icon_path
        interface["logo"] = icon_path
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--icon-path",
        help="Existing relative icon path to wire into the Codex manifest",
    )
    parser.add_argument("--brand-color", help="#RRGGBB brand color for --icon-path")
...
def main() -> None:
    ...
    icon_path = None
    brand_color = None
    if args.icon_path is not None:
        if args.brand_color is None:
            raise ValueError("--brand-color is required when --icon-path is used.")
        icon_path = validate_existing_icon_path(plugin_root, args.icon_path)
        brand_color = validate_brand_color(args.brand_color)
