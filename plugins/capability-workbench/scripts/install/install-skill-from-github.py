#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into the active agent's skills dir.

The destination resolves through agent_target.py (codex, claude, or cursor).
Override with --dest, --agent, or the AGENT_TARGET env var.
"""
...
for _agent_target in (
    ...
    if _agent_target.is_file():
        ...
        break
from agent_target import AGENTS, AgentResolutionError, resolve_agent  # noqa: E402
...
def _agent_skills_dir(agent: str | None = None) -> str:
    try:
        return str(resolve_agent(explicit=agent).skills_dir)
    except AgentResolutionError as exc:
        raise InstallError(
            f"cannot resolve the target agent ({exc}); pass --dest, --agent, or set AGENT_TARGET"
        ) from exc


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "agent-skill-install")
...
def _default_dest(agent: str | None = None) -> str:
    return _agent_skills_dir(agent)
...
def _parse_args(argv: list[str]) -> Args:
    ...
    parser.add_argument(
        "--agent",
        choices=AGENTS,
        help="Target agent for the default destination (defaults to the active agent)",
    )
    parser.add_argument(
