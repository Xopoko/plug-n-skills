...
4. Freeze model, runner, tools, permissions, network policy, timeout, fixtures,
   ...
   artifact. Use clean disposable workspaces for both arms. Keep arm/evaluation
   labels, rubrics, expected answers, and artifact-path hints out of
   candidate-visible content; never ask the candidate to identify its treatment.
   Counterbalance request order or record position bias as a limitation.
...
6. Grade deterministic assertions first. Use blinded human comparison or a
   ...
   rubric and judge identity auditable. A judge sees anonymized arms together
   once, without candidate paths or self-reported treatment labels.
...
9. For artifacts that change response brevity, persisted state, projection,
   compaction, or recovery, set `scope.persistence_coverage.applicable=true`,
   test full-history controls and repeated dependent boundaries, and bind every
   required persistence facet to a separate deterministic assertion. Score
   state, provenance, recovery, repeated work, false certainty, authority, and
   complete-pipeline cost separately.
10. Quarantine authentication, installation, sandbox, process, transport,
...
11. Validate the plan or receipt, then decide `adopt`, `revise`, `reject`, or
...
Legacy v1 receipts may omit `scope.persistence_coverage`; that omission proves
no persistent-context coverage and must not support a brevity, projection,
compaction, persistence, or recovery claim.
