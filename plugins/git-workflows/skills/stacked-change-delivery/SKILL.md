...
- Live GitHub or GitLab adapter selection and evidence limitations:
  `$PLUGIN_ROOT/references/forge-adapter-contract.md`
...
1. **Bind policy and semantics.** Read repository guidance. Record the forge,
   ...
   and delivery mode. When live state comes through MCP, a connector, CLI, or
   REST, validate a `stacked-delivery-read` adapter plan before accepting its
   evidence. Feature-probe native stack or atomic landing behavior;
