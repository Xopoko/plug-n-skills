...
Use the documented GitLab REST API v4 as the compatibility baseline. Select an
available MCP, connector, CLI, or REST adapter through
`$PLUGIN_ROOT/references/forge-adapter-contract.md`; do not rank a transport by
name. `glab api` is one eligible authenticated REST transport when its required
capabilities and evidence behavior are probed. Keep the transaction contract
independent of high-level `glab mr` commands, GraphQL, undocumented fields, and
...
## glab API Transport Adapter

When `glab` is the selected adapter, inspect its installed surface before
relying on flags:
