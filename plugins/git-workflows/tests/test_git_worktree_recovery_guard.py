...
@unittest.skipIf(
    os.name == "nt",
    "ready-state audit requires POSIX directory-relative symlink primitives",
)
...
class RetentionClassificationTests(FixtureTestCase):
    @unittest.skipIf(
        os.name == "nt",
        "ready-state retention proof requires the POSIX repair contract",
    )
...
@unittest.skipIf(
    os.name == "nt",
    "link mutation requires POSIX directory-relative symlink primitives",
)
class RepairTests(FixtureTestCase):
    ...
    def test_equivalent_but_different_raw_target_fails_postcondition(self):
        ...
        def substitute_equivalent_target(
            ...
            alternate_name = ".git-workflows-adversarial"
            os.symlink(
                str(self.fixture.replacement),
                alternate_name,
                dir_fd=descriptor,
...
class ProcessSafetyTests(FixtureTestCase):
    ...
    @unittest.skipIf(
        os.name == "nt",
        "successful audit assertion requires the POSIX readiness contract",
    )
