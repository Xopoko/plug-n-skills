...
class CliTests(unittest.TestCase):
    ...
    def test_hash_body_file_normalizes_newlines_without_echoing_body(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            # Preserve the fixture bytes on Windows instead of allowing text-mode
            # newline expansion to turn CRLF into CR-CRLF before the guard reads it.
            body_path.write_bytes(raw_body.encode("utf-8"))
            stdout = io.StringIO()
            with contextlib.redirect_stdout(stdout):
                code = guard.main(
