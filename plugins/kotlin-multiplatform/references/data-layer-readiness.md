...
| Cancellation-atomic dispatcher hop | After final admission/authority validation, a durable commit, its typed accepted receipt, and awaited required publication stay inside one outer same-dispatcher `NonCancellable` boundary; caller cancellation is checked immediately afterward | Let IO commit and queue its return on an instrumented caller dispatcher, cancel before that runnable executes, then release it; prove `wakeAccepted` names the exact receipt before cancellation emerges, the cancelled caller receives no typed return, and a later caller-owned effect does not run |
...
otherwise a cancellation request could no longer stop an unowned commit. Keep
the boundary narrow. The protected publisher must be a bounded local handoff
or durable enqueue. Call it directly and await an identity-bound
`WakeAccepted`; do not detach it as fire-and-forget work. Remote or retrying
delivery belongs behind the ordered idempotent notification record. If the
local handoff needs another dispatcher, nest that switch inside the same outer
boundary. After required acceptance,
...
identity-bound receipts. Let IO emit `durableCommitted` and `receiptReady`, then
finish so the continuation is queued on the caller dispatcher. Record
`returnQueued` without running that continuation, cancel the original caller,
then release it. Prove `wakeAccepted` identifies the exact typed receipt before
cancellation emerges. The cancelled caller receives no typed return and the
later caller-owned effect does not run. A no-cancellation control must return
the same receipt after `wakeAccepted`.
